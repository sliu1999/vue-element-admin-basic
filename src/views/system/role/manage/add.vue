        ...
        <el-dialog :visible.sync="addRoleModel" @close="closeDialog()" :close-on-click-modal="false" :modal-append-to-body="false" title="新增角色" :width="width">
            <el-form :model="addRoleInfo" :rules="rules" ref="addRoleInfo" label-position="top" class="demo-ruleForm padding20">
                 <el-form-item label="用户名" prop="roleName">
                    <el-input placeholder="请输入角色名" v-model="addRoleInfo.roleName"></el-input>
                </el-form-item>
                <el-form-item label="角色类型" prop="roleType">
                    <el-select v-model="addRoleInfo.roleType" placeholder="请选择角色类型">
                    <el-option v-for="item in roleTypeList" :key="item.value" :label="item.label" :value="item.value">
                    </el-option>
                </el-select>
                </el-form-item>
                <el-form-item label="停车场" prop="parkId">
                     <el-select v-model="addRoleInfo.parkId" placeholder="请选择">
                        <el-option
    ...
    methods: {
        ...
        save(){
            this.$refs['addRoleInfo'].validate( valid => {
                if(valid){
                    roleApi.saveRole(this.addRoleInfo)
                        .then(res => {
                            this.closeDialog()
                            this.$parent.init()
